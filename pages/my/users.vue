        ...
        <v-card class="admin__card">
            ...
            <div class="user" v-for="user in users" v-if="user.Email !== 'admin@admin.com'">
                <div class="section">
                    ...
                    >{{ mapUserKey('EmailConfirmed') }}</p>
                    <v-btn
                        v-else
                        small
                        color="primary"
                        v-bind:style="{ margin: 0, width: 90 }"
                        @click="sendUserConfirmationEmail(user)"
                    >Потвърди</v-btn>
                </div>
                <div class="section section-centered">
                    <div style="width: 210px; position: relative;right: -10px;bottom: -13px;">
                        <vuex-switch
                            v-bind:label="mapUserKey('OnlyHighCostVisible')"
                            :model="user['OnlyHighCostVisible']"
                            @change="changeUserHighCostVisibility(user, ...arguments)"
                        ></vuex-switch>
                    </div>
                </div>
                <div class="section section-centered">
                    <div style="width: 230px; position: relative;right: -10px;bottom: -13px;">
                        <vuex-switch
                            v-bind:label="mapUserKey('IsDeferredPaymentAllowed')"
                            :model="user['IsDeferredPaymentAllowed']"
                            @change="changeUserAllowedDeferredPayment(user, ...arguments)"
                        ></vuex-switch>
                    </div>
                </div>
                <div class="section section-centered">
                    <p>Намаление:&nbsp;</p>
                    <vuex-input
                        :model="user.PercentageReduction"
                        :styles="{width: '2em', fontSize: '1.5em'}"
                        @change="changeUserPercentageReduction(user, ...arguments)"
                    ></vuex-input>
                    <p>%</p>
                </div>
            </div>
...
export default {
    ...
    methods: {
        ...
        mapUserKeyValue(key, value) {
            if (
                key === "OnlyHighCostVisible" ||
                key === "EmailConfirmed" ||
                key === "IsDeferredPaymentAllowed"
            ) {
                return value ? "Да" : "Не";
            } else {
                return value;
        ...
        sendUserConfirmationEmail(user) {
            this.$store.dispatch("modules/auth/sendUserConfirmationMail", {
                user
            });
        },
        changeUserHighCostVisibility: _.debounce(function(user, visibility) {
            this.$store.dispatch("modules/auth/setUserHighCostVisibility", {
                user,
                visibility
            });
        }, 300),
        ...
        changeUserAllowedDeferredPayment: _.debounce(function(user, allowed) {
            this.$store.dispatch("modules/auth/setUserAllowedDeferredPayment", {
                user,
                allowed
            });
        }, 300),
        changeUserPercentageReduction: _.debounce(function(user, percentage) {
            this.$store.dispatch("modules/auth/setUserPercentageReduction", {
                user,
                percentage
            });
        }, 300)
