								...
								<v-flex xs12 column>
									<v-flex xs12 column>
										<v-card-text>
											...
											<div style="clear:both">
												<v-chip v-if="curProductDetails.HasFittingMethod">{{ "HasFittingMethod" }}</v-chip>
												<v-chip v-if="curProductDetails.IsAcoustic">{{ "IsAcoustic" }}</v-chip>
												<v-chip v-if="curProductDetails.IsCalibration">{{ "IsCalibration" }}</v-chip>
											</div>
											<div v-if="hasAnyAvailability()">
												...
												<v-flex row class="text-xs-center" style="margin-bottom:10px">
													<div v-for="group in currentAccessoryAvailability"
	...
	export default {
		...
		computed: {
			currentAccessoryAvailability() {
				return this.$store.getters["modules/products/getCurrentObservedAccessoryAvailability"];
			}
		},
		methods: {
			...
			hasAnyAvailability() {
				console.log(this.currentAccessoryAvailability);
